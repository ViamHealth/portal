...
USER_ALERT_EMAILS = ['sharat.khurana@viamhealth.com',]
...
def send_mail(subject, message, recipient_list, from_email=None, attachment=None ):
	...
	from_email = 'Team ViamHealth <' + FROM_VIAM_EMAIL + '>'
...
def signup_email(email):
	...
	send_mail('New User Sign Up Alert','Email Id - '+ str(email), USER_ALERT_EMAILS)

def invite_new_email(invited, inviter, password):
	email = invited.email
	subject = 'Have a Healthy Future'
        message = 'Hello,<br/><br/>You have been invited to ViamHealth.<br/><br/>Your registration details -<br/><br/>UserName: ' + str(email) + '<br/><br/>Password(autogenerated): '+ password + '<br/><br/>Please change the password as soon as you login.<br/><br/>Please do provide us with any feedback on feedback@viamhealth.com.<br/><br/>We wish you and your family a healthy future.'+ Email_footer
	send_mail(subject, message, [email])
	send_mail('New User Invite Alert','Invited Email Id - '+ str(email), USER_ALERT_EMAILS)

def invite_existing_email(invited, inviter):
	email = invited.email
	subject = 'Have a Healthy Future'
        message = 'Hello,<br/><br/>You have been invited to ViamHealth.<br/><br/>Your registration details -<br/><br/>UserName: ' + str(email) + '<br/><br/>Please do provide us with any feedback on feedback@viamhealth.com.<br/><br/>We wish you and your family a healthy future.'+ Email_footer
	send_mail(subject, message, [email])
	send_mail('Existing User Invite Alert','Invited Email Id - '+ str(email), USER_ALERT_EMAILS)
...
def forgot_password_email(user,password):
	...
	send_mail(subject, message, [email])
	send_mail('Forgost password request Alert','Email Id - '+ str(email), USER_ALERT_EMAILS)

def share_user_email(invited, inviter, share_user):
	same_user = False
	if share_user.username == inviter.username:
		same_user = True
	...
	gender_str_1 = share_user.first_name + '\'s'
	if same_user:
		if share_user.profile.gender is not None:
			if share_user.profile.gender == 'MALE':
				gender_str_1 = 'his'
			elif share_user.profile.gender == 'FEMALE':
				gender_str_1 = 'her'

	gender_str_2 = share_user.first_name
	if same_user:
		if share_user.profile.gender is not None:
			if share_user.profile.gender == 'MALE':
				gender_str_2 = 'He'
	...
	subject = share_user.first_name + '\'s health profile has been shared with you'
	message = 'Hello'+invited_name_str+',<br/><br/>'+inviter_name_str+' wants to share '+ gender_str_1 +' health profile.' + gender_str_2 + ' will also get access to your health profile.<br/><br/>We wish both of you a healthy future.<br/><br/>Please do provide us with any feedback on feedback@viamhealth.com.'+Email_footer
	inv_email = None
	if inviter.email is not None:
		inv_email = inviter.email
	send_mail(subject, message, [email], inv_email)
	send_mail('Share request Alert','Invited Name - '+ invited_name_str + '<br/>Invited Name-' + inviter_name_str + '<br/>Shared user - ' + gender_str_1, USER_ALERT_EMAILS)
